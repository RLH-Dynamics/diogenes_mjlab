...
Run from the repo root (so the mesh assets resolve):
    ...
    python -m diogenes_mjlab.tools.inspect_joints --xml src/diogenes_mjlab/diogenes/xmls/scene.xml
    python -m diogenes_mjlab.tools.inspect_joints --freeze-slider     # weld the carriage so only
                                                 # hip/thigh/calf move
...
from ._shared import load_model_from_xml, get_joint_id, get_joint_name


def main() -> None:
    """Launch the interactive joint inspector viewer."""
    ...
    model = load_model_from_xml(args.xml)
